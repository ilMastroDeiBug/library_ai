...
import 'package:library_ai/models/ai_hero_banner.dart';
...
class _MovieDetailPageState extends State<MovieDetailPage> {
  ...
  void initState() {
    ...
    _imagesFuture = sl<TmdbService>().fetchMediaImages(_id, isTv: _isTv);
  }
                                ...
                                child: ElevatedButton(
                                  ...
                                  onPressed: () async {
                                    ...
                                    try {
                                      ...
                                      setState(() {
                                        errorMessage =
                                            "Errore durante la generazione: ${e.toString().replaceAll('Exception: ', '')}";
                                        isLoading = false;
                                      });
                                    }
                                  },
  ...
  Future<dynamic>? _getMediaFuture(String userId) async {
    try {
      if (_isTv) {
        return await sl<GetSingleTvSeriesUseCase>()
            .call(userId, _id.toString())
            .first;
      }
      return await sl<GetSingleMovieUseCase>()
          .call(userId, _id.toString())
          .first;
    } catch (e) {
      return null;
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<dynamic>(
      ...
      builder: (context, snapshot) {
        ...
        final String releaseDate = _isTv
            ? liveMedia.firstAirDate
            : liveMedia.releaseDate;
          ...
          body: Stack(
            children: [
              CustomScrollView(
                physics: const BouncingScrollPhysics(),
                slivers: [
                  // ── Banner come sliverToBoxAdapter normale ─────────────────
                  // NOTA: non usare SliverAppBar — il CustomScrollView
                  // intercetterebbe i gesti orizzontali del PageView interno.
                  SliverToBoxAdapter(
                    child: FutureBuilder<List<String>>(
                      future: _imagesFuture,
                      builder: (context, snapshot) {
                        final extraImages = snapshot.data ?? [];
                        return _buildBanner(
                          (poster.isNotEmpty ? poster : backdrop).replaceAll(
                            'w500',
                            'original',
                          ),
                          extraImages,
                      ...
                      child: Column(
                        ...
                        children: [
                                  ...
                                  children: [
                                    _buildIconBtn(
                                      ...
                                      color: _isFavorite
                                          ...
                                          : Colors.white,
                                      onTap: () =>
                                          _handleFavoriteToggle(liveMedia),
                                    ),
                                    const SizedBox(width: 12),
                                    _buildIconBtn(
                                      ...
                                      color: Colors.white,
                                      onTap: () =>
                                          _showCollectionsBottomSheet(title),
                                    ),
                                    const SizedBox(width: 12),
                                    _buildIconBtn(
                                      ...
                                      color: Colors.white,
                                      onTap:
                                          () {}, // Implementare share se necessario
                                    ),
                                  ],
                                ),
                            ],
                          ...
                          Wrap(
                            ...
                            children: [
                              ...
                              if (_isTv && seasonsMap.isNotEmpty)
                                _GlassChip(
                                  text: '${seasonsMap.length} Stagioni',
                                ),
                          ...
                          if (productionStatus.isNotEmpty)
                            _GlassChip(
                              text:
                                  'Produzione: ${productionStatus.toUpperCase()}',
                            ),
                          const SizedBox(height: 24),
                          ...
                          if (genres.isNotEmpty) ...[
                            Wrap(
                              ...
                              runSpacing: 8,
                              children: genres
                                  .map((g) => _GlassChip(text: g, dark: true))
                                  .toList(),
                            ),
                            const SizedBox(height: 24),
                          ],
              ...
              // ── Back button sempre visibile sovrapposto ─────────────────────
              Positioned(
  ...
  /// Builds the image banner as a plain [Widget] (NOT inside [SliverAppBar]).
  /// Keeping it outside the sliver tree is the only reliable way to let the
  /// inner [PageView] receive horizontal swipe gestures without the parent
  /// [CustomScrollView] capturing them first.
  Widget _buildBanner(String imageUrl, List<String> extraImages) {
    final String title = _isTv
        ? (widget.media as TvSeries).name
        : (widget.media as Movie).title;
    final allImages = [imageUrl, ...extraImages].take(10).toList();
    final dummyItems = allImages.map((img) {
      if (_isTv) {
        return TvSeries(
          id: widget.media.id,
          name: title,
    ...
    return SizedBox(
      height: 420,
      child: AiHeroBanner(items: dummyItems, onItemTap: (_) {}, showCta: false),
    );
  }

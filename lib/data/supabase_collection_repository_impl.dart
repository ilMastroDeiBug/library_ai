...
class SupabaseCollectionRepositoryImpl implements CollectionRepository {
  ...
  // CACHE LOCAL
  final Map<String, StreamController<List<MediaCollection>>> _collectionsControllers = {};
  final Map<String, List<MediaCollection>> _cachedCollections = {};

  final Map<int, StreamController<List<MediaCollectionItem>>> _itemsControllers = {};
  ...
  Stream<List<MediaCollection>> getCollectionsStream(String userId) async* {
    ...
    if (!_collectionsControllers.containsKey(userId)) {
      _collectionsControllers[userId] = StreamController<List<MediaCollection>>.broadcast();
    }
      ...
      for (var row in response) {
        try {
          ...
          if (itemsResponse != null) {
            ...
            if (type == 'movie') {
              final mediaResp = await _supabase.from('movies').select('poster_path').eq('id', int.parse(itemIdStr)).maybeSingle();
              if (mediaResp != null && mediaResp['poster_path'] != null) {
                coverUrl = 'https://image.tmdb.org/t/p/w500${mediaResp['poster_path']}';
              }
            } else if (type == 'tv') {
              final mediaResp = await _supabase.from('tv_series').select('poster_path').eq('id', int.parse(itemIdStr)).maybeSingle();
              if (mediaResp != null && mediaResp['poster_path'] != null) {
                coverUrl = 'https://image.tmdb.org/t/p/w500${mediaResp['poster_path']}';
              }
            } else if (type == 'book') {
              final mediaResp = await _supabase.from('books').select('thumbnail_url').eq('id', itemIdStr).maybeSingle();
              if (mediaResp != null && mediaResp['thumbnail_url'] != null) {
                coverUrl = mediaResp['thumbnail_url'];
              }
            }
          }
  ...
  @override
  Future<MediaCollection> createCollection(String userId, String name, {String? description}) async {
    ...
    if (_cachedCollections.containsKey(userId)) {
      _cachedCollections[userId] = [newCollection, ..._cachedCollections[userId]!];
  ...
  Future<void> deleteCollection(int collectionId) async {
    ...
    if (foundUserId != null) {
      ...
      _collectionsControllers[foundUserId]?.add(_cachedCollections[foundUserId]!);
    }
  }

  @override
  Future<void> addMediaToCollection(int collectionId, String itemId, String itemType) async {
  ...
  @override
  Future<void> removeMediaFromCollection(int collectionId, String itemId, String itemType) async {
    await _supabase
        .from('collection_items')
        .delete()
  ...
  @override
  Stream<List<MediaCollectionItem>> getCollectionItemsStream(int collectionId) async* {
    final initialItems = await _fetchCollectionItems(collectionId);
    _cachedItems[collectionId] = initialItems;
    yield initialItems;

    if (!_itemsControllers.containsKey(collectionId)) {
      _itemsControllers[collectionId] = StreamController<List<MediaCollectionItem>>.broadcast();
    }
    yield* _itemsControllers[collectionId]!.stream;
  }

  Future<List<MediaCollectionItem>> _fetchCollectionItems(int collectionId) async {
    try {
      final response = await _supabase
          .from('collection_items')
          .select()
      ...
      for (var row in response) {
        try {
          ...
          if (type == 'movie') {
            final mediaResp = await _supabase.from('movies').select('title, poster_path').eq('id', int.parse(itemIdStr)).maybeSingle();
            if (mediaResp != null) {
              title = mediaResp['title'];
              if (mediaResp['poster_path'] != null) {
                posterUrl = 'https://image.tmdb.org/t/p/w500${mediaResp['poster_path']}';
              }
            }
          } else if (type == 'tv') {
            final mediaResp = await _supabase.from('tv_series').select('name, poster_path').eq('id', int.parse(itemIdStr)).maybeSingle();
            if (mediaResp != null) {
            ...
            final mediaResp = await _supabase.from('books').select('title, thumbnail_url').eq('id', itemIdStr).maybeSingle();
            if (mediaResp != null) {
              title = mediaResp['title'];
              posterUrl = mediaResp['thumbnail_url'];
            }
          }

          items.add(MediaCollectionItem(
            id: row['id'],
  ...
  @override
  Future<List<int>> getItemCollectionIds(String userId, String itemId, String itemType) async {
    ...
    final List<int> userColIds = (collections as List).map((c) => c['id'] as int).toList();
